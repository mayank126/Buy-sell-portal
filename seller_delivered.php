...
<?php
        $query="Select O.*,B.name,B.phoneno ,P.name AS prodname FROM project.orders O, project.buyer B , project.product P WHERE P.sellerid=\"$seller\"and O.productid=P.productid and B.buyerid=O.buyerid and B.subid=\"PRE\" and O.status='done'"; 
        ...
        echo "<div class=\"container-md mt-5 \">";
            ...
            $query="Select O.*,B.name,B.phoneno, P.name AS prodname FROM project.orders O, project.buyer B , project.product P WHERE P.sellerid=\"$seller\"and O.productid=P.productid and B.buyerid=O.buyerid and B.subid=\"BA\" and O.status='done'"; 
        $sql=$conn->prepare($query);
        $sql->execute();
        if($sql->rowCount())
            {   $cnt+=$sql->rowCount(); 
